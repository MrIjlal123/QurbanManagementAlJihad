...
try {
    switch ($action) {
        case 'getKeuangan':
            ...
            $stmt = $pdo->prepare('SELECT id, tahun, tanggal, jenis, ekor, harga, nominal, keterangan FROM qurban_keuangan ORDER BY tanggal DESC LIMIT ' . intval($pag['limit']) . ' OFFSET ' . intval($pag['offset']));
            $stmt->execute();
        ...
        case 'getDistribusi':
            ...
            if ($year !== '') {
                ...
                $stmt = $pdo->prepare('SELECT id, tahun, kategori, sumber, label, rt, berat_per_bungkus, jumlah_bungkus, total_berat, hewan_id, jumlah_sohibul, berat_per_sohibul FROM qurban_distribusi WHERE tahun = ? ORDER BY created_at ASC LIMIT ' . intval($pag['limit']) . ' OFFSET ' . intval($pag['offset']));
                $stmt->execute([$year]);
            } else {
                $total = getTotalRows($pdo, 'qurban_distribusi');
                $stmt = $pdo->prepare('SELECT id, tahun, kategori, sumber, label, rt, berat_per_bungkus, jumlah_bungkus, total_berat, hewan_id, jumlah_sohibul, berat_per_sohibul FROM qurban_distribusi ORDER BY created_at ASC LIMIT ' . intval($pag['limit']) . ' OFFSET ' . intval($pag['offset']));
                $stmt->execute();
            }
            echo json_encode(responsePaginated($stmt->fetchAll(), $pag['page'], $pag['limit'], $total));
            break;
        ...
        case 'updateHewan':
            // validate owners and normalize year
            ...
            if ($jenis === 'SAPI') {
                if ($ownerCount < 1 || $ownerCount > 7) { echo json_encode(['success'=>false,'message'=>'Sapi harus memiliki 1 sampai 7 pemilik (sebutkan nama masing-masing).']); break; }
            } else {
                // Kambing/Domba must be exactly 1
                if ($ownerCount !== 1) { echo json_encode(['success'=>false,'message'=>'Kambing/Domba harus memiliki tepat 1 pemilik.']); break; }
            }
        ...
        case 'addKeuangan':
            ...
            $stmt->execute([$input['tahun']??'', $input['tanggal']??'', $input['jenis']??'', $input['ekor']??0, $input['harga']??0, $input['keterangan']??'', $input['nominal']??0]);
            echo json_encode(['success' => true]); break;
        ...
        case 'addHewan':
            ...
            $stmt->execute([$input['tahun']??'', $input['jenis']??'', $pemilikStr, $input['rt']??'', $input['permintaan']??'', $input['kotor']??0, $input['daging']??0, $input['keterangan']??'']);
            echo json_encode(['success' => true]); break;

        case 'addPenerima':
            validateYearOrFail($input['tahun'] ?? '', 'tahun');
            $stmt = $pdo->prepare('INSERT INTO qurban_penerima (tahun, nama, kategori, rt, permintaan, status) VALUES (?,?,?,?,?,?)');
            $stmt->execute([$input['tahun']??'', $input['nama']??'', $input['kategori']??'', $input['rt']??'', $input['permintaan'] ?? '-', $input['status']??'Belum Diambil']);
            echo json_encode(['success' => true]); break;
        ...
        case 'deleteDistribusi':
            $pdo->prepare('DELETE FROM qurban_distribusi WHERE id = ?')->execute([$_GET['id'] ?? 0]);
